# lassaVacc_HCWs
Model evaluating health impacts of vaccinating Nigerian healthcare workers against Lassa fever. Composed of 5 scripts and 5 data input files. 

This code was developed, tested and run in R v.4.5.1. All packages required to run the code are listed in housekeeping.R. Total run time on a normal desktop computer is <10 minutes. 

Running this code reproduces all quantitative results and figures in the manuscript. Note that one previously generated input dataset representing age- and sex-stratified LASV zoonosis incidence at the Admin1 level (df_incidence_2019_allAges.Rdata) must be generated following https://github.com/drmsmith/lassaRiskVac (infections_foi.R)

### (1) housekeeping.R
Base functions and labels

### (2) nigeria_data.R
NCDC case data time series and summary statistics

### (3) hospital_model_general_inputs.R
General model inputs; this loads from the /data directory, including data from a literature review of Lassa fever CFR in HCWs (review_data_extraction.xlsx), state-level population estimates (df_population.Rdata), life expectancy data (df_un_le_nigeria_2025.csv) and decision tree parameters (params_montecarlo.Rdata)

### (4) hospital_model_inputs_approach3.R
The model, simulates outcomes and vaccine impact

### (5) hospital_model_summarize.R
Summarizes simulation results

### (6) hospital_model_tables_plots.R
Data tabulation and visualisation
